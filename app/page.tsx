...
const tools: Tool[] = [
  ...
  { id: "slides", title: "Sketch-to-Slide", description: "Bild oder Skizze in professionelle PowerPoint-Folien überführen.", source: "Lokaler Rechner", status: "Aktiv", category: "Präsentation", detail: "Lokale Anwendung mit Bild-Upload, KI-Inhalt und PowerPoint-Export.", location: "C:\\2026\\Claude\\PPT Bilderstellung\\sketch-to-slide" },
...
const screenshots = allTools.slice(0, 10).map((tool, index) => ({ src: `/screenshots/master-${index + 1}.png`, title: tool.title, source: "Master-App" }));

function screensFor(tool: Tool) {
  if (tool.id === "messe") return [{ src: "/screenshots/ai-messe-guide.png", title: "Live Dashboard", source: "Echte Live-App" }, ...screenshots.slice(0, 9)];
  return screenshots;
}

type AppDetails = { builder: string; frontend: string; middleware: string; backend: string; database: string; connections: string; models: string; evidence: string };
...
const appDetails: Record<string, AppDetails> = {
  ...
  "contracts-hub": { builder: "Noch nicht aus Repository-Code verifiziert", frontend: "Noch zu pruefen", middleware: "Noch zu pruefen", backend: "Noch zu pruefen", database: "Noch zu pruefen", connections: "Privates GitHub-Repository", models: "Noch zu pruefen", evidence: "GitHub-Repository-Metadaten" },
  ...
  thumbnail: { builder: "Wahrscheinlich Claude; Abgleich mit Presentation Designer offen", frontend: "Aus Repository-Code pruefen", middleware: "Aus Repository-Code pruefen", backend: "Aus Repository-Code pruefen", database: "Aus Repository-Code pruefen", connections: "GitHub-Repository; moegliche Ueberschneidung mit Presentation Designer", models: "Noch nicht aus dem Repository verifiziert", evidence: "GitHub-Metadaten und lokaler Namensabgleich" },
  ...
  dify: { builder: "Dify / genauer Ursprung noch pruefen", frontend: "Aus Repository-Code pruefen", middleware: "Dify- und Workflow-Anbindung vermutet", backend: "Aus Repository-Code pruefen", database: "Noch zu pruefen", connections: "GitHub-Repository und Releaseletter-Cluster", models: "Dify-Modellkonfiguration noch nicht verifiziert", evidence: "GitHub-Metadaten und lokale Workflow-Verbindung" },
  ...
  event: { builder: "Noch nicht aus Repository-Code verifiziert", frontend: "Noch zu pruefen", middleware: "Noch zu pruefen", backend: "Noch zu pruefen", database: "Noch zu pruefen", connections: "GitHub-Repository; fachliche Naehe zum AI Messe Guide", models: "Noch zu pruefen", evidence: "GitHub-Metadaten und lokaler Projektabgleich" },
  messe: { builder: "Claude", frontend: "Statisches HTML, CSS und JavaScript", middleware: "Optionales Supabase Cloud-Modul", backend: "Supabase Auth, Sync und Konto-Loeschung", database: "Supabase PostgreSQL optional; lokal LocalStorage und IndexedDB", connections: "GitHub zu Vercel Deployment, Supabase MFA, Cloud-Sync und Row-Level Security", models: "Kein LLM im App-Code dokumentiert", evidence: "README, cloud.js, schema.sql, vercel.json und Live-Screen" },
      ...
      {opened && <div className="app-window-layer" role="presentation">
        <section className="app-window" role="dialog" aria-modal="true" aria-label={`${opened.title} Arbeitsfenster`} style={{ transform: `translate(calc(-50% + ${windowPosition.x}px), calc(-50% + ${windowPosition.y}px))` }}>
          ...
          <nav className="window-tabs" aria-label="App-Informationen">
            ...
            <button className={windowTab === "screens" ? "active" : ""} onClick={() => setWindowTab("screens")}>Screenshots (10)</button>
          </nav>
          <div className="window-content">
            {windowTab === "profile" && <div className="window-profile"><div><p className="eyebrow">Entwickelt mit</p><h2>{detailsFor(opened).builder}</h2><p>{opened.detail}</p></div><dl className="window-data"><div><dt>Quelle</dt><dd>{opened.source}</dd></div><div><dt>Startpunkt</dt><dd>{opened.location}</dd></div><div><dt>Modell</dt><dd>{detailsFor(opened).models}</dd></div><div><dt>Verbindungen</dt><dd>{detailsFor(opened).connections}</dd></div><div><dt>Pruefgrundlage</dt><dd>{detailsFor(opened).evidence}</dd></div><div><dt>Verwandte Apps</dt><dd>{opened.overlap ?? "Noch abgleichen"}</dd></div></dl></div>}
            ...
            {windowTab === "screens" && <div><p className="screens-intro">Echte Ansichten werden bevorzugt angezeigt. Wenn eine Ziel-App noch nicht direkt erreichbar ist, bleibt die gespeicherte Master-App-Ansicht sichtbar und klar gekennzeichnet.</p><div className="screens-grid">{screensFor(opened).map((screen) => <figure key={screen.src}><img src={screen.src} alt={`Bildschirmansicht ${screen.title}`} /><figcaption>{screen.title}<span>{screen.source}</span></figcaption></figure>)}</div></div>}
          </div>
        </section>
      </div>}
    </main>
