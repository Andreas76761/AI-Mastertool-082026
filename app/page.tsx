...
type AgentSetup = { agentKey: string; deviceKey: string; agentName: string; appKey: string; port: string };
type AgentEnrollment = { code: string; expiresAt: string; command: string };
...
const chatRecords: ChatRecord[] = [
  ...
  { id: "claude-wasserschaden", source: "Claude Desktop", title: "Wasserschaden-Chatakte", summary: "Sechs lokale PDF-Artefakte zu Angebot, Rechnung, E-Mail-Verkehr und Rechtsschutzkommunikation.", updatedAt: "20.07.2026 13:03", model: "In den lokalen Artefakten nicht ausgewiesen", appId: "claude-workspace-wasserschaden", appMatch: "Wasserschaden-Chatakte", status: "Erfasst" },
];
...
const chatProviderStates = [
  ...
  { source: "Claude Desktop", state: "Teilzugriff", detail: "App läuft; lokale Artefakte geprüft, Chat-Historie nicht lokal exportiert" },
...
const claudeWorkspaceTools: Tool[] = [
  ...
  { id: "claude-workspace-wasserschaden", title: "Wasserschaden-Chatakte", description: "Claude-Artefakt mit sechs PDF-Unterlagen zu Versicherungs- und Schadenskommunikation.", source: "Lokaler Rechner", status: "Dokumentiert", category: "Verträge", detail: "Lokale Claude-Artefaktakte; kein eigenständiger App-Startpunkt, aber klarer Dokumentenbestand.", location: "C:\\2026\\Claude\\-Chat Wasserschaden", createdAt: "10.07.2026", checkedAt: "11.08.2026" },
  { id: "claude-workspace-messe-pm", title: "Messe PM Planungsartefakt", description: "Excel- und JSON-Artefakt zur Messeplanung.", source: "Lokaler Rechner", status: "Dokumentiert", category: "Organisation", detail: "Kleiner lokaler Planungsbestand mit Excel-Arbeitsmappe und JSON-Datei; keine Web-App erkannt.", location: "C:\\2026\\Claude\\Messe PM", createdAt: "05.07.2026", checkedAt: "11.08.2026" },
  { id: "claude-workspace-testing-guide", title: "Testing Guide", description: "Angelegter Claude-Projektordner ohne gegenwärtigen Inhalt.", source: "Lokaler Rechner", status: "Entwurf", category: "Dokumentation", detail: "Projektordner ist vorhanden, enthält bei der lokalen Prüfung aber keine Dateien oder App-Artefakte.", location: "C:\\2026\\Claude\\Testing_Guide", createdAt: "06.07.2026", checkedAt: "11.08.2026" },
];
...
export default function Home() {
  ...
  const [agentSetup, setAgentSetup] = useState<AgentSetup>({ agentKey: "pc-2-agent", deviceKey: "pc-2", agentName: "PC 2", appKey: "overview", port: "3000" });
  const [agentEnrollment, setAgentEnrollment] = useState<AgentEnrollment | null>(null);
  const [agentNotice, setAgentNotice] = useState("Für einen neuen Rechner wird ein zeitlich begrenzter Einmal-Code erzeugt.");
  ...
  const costByApp = useMemo(() => Object.values(costEntries.reduce<Record<string, { key: string | null; title: string; totalEur: number; totalUsd: number; entries: number }>>((groups, entry) => { const key = entry.app_key ?? "unassigned"; const tool = catalogTools.find((item) => item.id === entry.app_key); const group = groups[key] ?? { key: entry.app_key, title: tool?.title ?? "Nicht zugeordnet", totalEur: 0, totalUsd: 0, entries: 0 }; if (entry.currency === "EUR") group.totalEur += Number(entry.amount); if (entry.currency === "USD") group.totalUsd += Number(entry.amount); group.entries += 1; groups[key] = group; return groups; }, {})).sort((a, b) => (b.totalEur + b.totalUsd) - (a.totalEur + a.totalUsd)), [catalogTools, costEntries]);
  const totalCosts = costEntries.filter((entry) => entry.currency === "EUR").reduce((sum, entry) => sum + Number(entry.amount), 0);
  const totalUsdCosts = costEntries.filter((entry) => entry.currency === "USD").reduce((sum, entry) => sum + Number(entry.amount), 0);
  ...
  async function createAgentEnrollment() {
    setAgentNotice("Einmal-Aktivierung wird sicher vorbereitet …");
    ...
    try {
      ...
      setAgentNotice(error instanceof Error ? error.message : "Einmal-Aktivierung konnte nicht erstellt werden.");
    }
  }
  ...
  return (
    <main className="app-shell">
      ...
      {mainView === "costs" && <section className="costs-view" aria-label="Kostenübersicht">
        <div className="dashboard-heading"><div><p className="eyebrow">Kostenakte ab Januar 2025</p><h1>Alle Tool-Kosten im Blick.</h1><p>Die Ansicht zeigt ausschließlich importierte Zahlungszeilen. Es werden keine Kosten geschätzt oder aus Zugangsdaten abgeleitet.</p></div><div className="documentation-summary"><span><strong>{new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(totalCosts)}</strong> Gesamt, EUR</span><span><strong>{new Intl.NumberFormat("de-DE", { style: "currency", currency: "USD" }).format(totalUsdCosts)}</strong> USD, ohne Umrechnung</span><span><strong>{costEntries.length}</strong> Zahlungszeilen</span></div></div>
        ...
        <div className="cost-layout"><section className="cost-panel"><h2>Monatliche Entwicklung</h2><div className="cost-bars">{monthlyCosts.map((month) => <div key={month.month}><span style={{ height: `${Math.max(4, totalCosts ? (month.total / Math.max(...monthlyCosts.map((entry) => entry.total), 1)) * 100 : 4)}%` }} title={`${month.month}: ${month.total.toFixed(2)} EUR`}></span><small>{month.month}</small><b className={month.change > 0 ? "up" : month.change < 0 ? "down" : "flat"}>{month.change > 0 ? "↑" : month.change < 0 ? "↓" : "–"}</b></div>)}</div></section><section className="cost-panel"><h2>Kosten nach App</h2>{costByApp.length ? <div className="cost-app-list">{costByApp.map((item) => <button key={item.title} type="button" onClick={() => item.key && (setSelectedId(item.key), setMainView("catalog"))}><span>{item.title}<small>{item.entries} Zeilen</small></span><strong>{item.totalEur > 0 && new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(item.totalEur)}{item.totalEur > 0 && item.totalUsd > 0 && " · "}{item.totalUsd > 0 && new Intl.NumberFormat("de-DE", { style: "currency", currency: "USD" }).format(item.totalUsd)}</strong></button>)}</div> : <p className="empty-costs">Nach dem CSV-Import erscheinen hier die Gesamtkosten pro App.</p>}</section></div>
      ...
      {mainView === "control" && controlTab === "security" && <section className="agent-enrollment control-panel" aria-label="Rechner aktivieren"><h2>Neuen Rechner aktivieren</h2><p>Erstellt einen einmaligen Code für den lokalen Status-Agenten. Das eigentliche Zugangsmerkmal wird nur auf dem jeweiligen Rechner gespeichert.</p><div className="agent-form"><input value={agentSetup.agentKey} onChange={(event) => setAgentSetup((current) => ({ ...current, agentKey: event.target.value }))} aria-label="Agentenkennung" placeholder="Agentenkennung" /><input value={agentSetup.deviceKey} onChange={(event) => setAgentSetup((current) => ({ ...current, deviceKey: event.target.value }))} aria-label="Gerätekennung" placeholder="Gerätekennung" /><input value={agentSetup.agentName} onChange={(event) => setAgentSetup((current) => ({ ...current, agentName: event.target.value }))} aria-label="Gerätename" placeholder="Name des Rechners" /><select value={agentSetup.appKey} onChange={(event) => setAgentSetup((current) => ({ ...current, appKey: event.target.value }))} aria-label="Zu prüfende App">{catalogTools.map((tool) => <option value={tool.id} key={tool.id}>{tool.title}</option>)}</select><input value={agentSetup.port} inputMode="numeric" onChange={(event) => setAgentSetup((current) => ({ ...current, port: event.target.value }))} aria-label="Lokaler Port" placeholder="Port" /><button type="button" onClick={createAgentEnrollment}>Einmal-Code erstellen</button></div><p className="security-notice">{agentNotice}</p>{agentEnrollment && <div className="agent-command"><strong>Auf dem neuen Rechner einmal ausführen:</strong><code>{agentEnrollment.command}</code><button type="button" onClick={() => navigator.clipboard?.writeText(agentEnrollment.command)}>Startbefehl kopieren</button><small>Code gültig bis {new Intl.DateTimeFormat("de-DE", { dateStyle: "short", timeStyle: "short" }).format(new Date(agentEnrollment.expiresAt))}. Nach erfolgreichem Start ist er ungültig.</small></div>}</section>}
